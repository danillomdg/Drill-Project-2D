...
public class Wormill : Enemy {
	...
	public void Deactivate()
	{
		...
		{
			try {
				tScript.blocks[Mathf.FloorToInt(transform.position.x),-1*Mathf.FloorToInt(transform.position.y)-1] = 23;
			}
	...
	public void Move (){
		...
		if (following == false)
		{
			...
			try
			{
				...
				if (verifyPassive1 != 0 && verifyPassive1 != 23)
				{
				...
				else 
				{
					...
					if (faceUD != 0)
					{
						...
						if ((verifyActive1 == 0 || verifyActive1 == 23 ) && esquina == false)
						{
					...
					else
					{
						...
						if ((verifyActive2 == 0 || verifyActive2 == 23) && esquina == false)
						{
